...
layui.define(["element", "jquery"], function (exports) {
    ...
    Tab.prototype.render = function () {
        ...
        $.get(url, function (data) {
            ...
            if ($("#navBar").html() == '') {
                var _this = this;
                $("#navBar").html(navBar(data)).height($(window).height() - 205);
                element.init();  //初始化页面元素
                $(window).resize(function () {
                    $("#navBar").height($(window).height() - 205);
                })
            }
        })
    }
        ...
        if (_this.attr("target") == "_blank") {
            ...
            if (that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0) {
                if ($(".layui-tab-title.top_tab li").length == openTabNum) {
                    layer.msg('为保证系统性能，你只能同时打开' + (openTabNum - 1) + '个选项卡！');
                    return;
                }
                tabIdIndex++;
                title += '<i class="' + _this.find("i").attr("data-icon") + '" aria-hidden="true"></i>';
                title += '<cite>' + _this.find("cite").text() + '</cite>';
                title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="' + tabIdIndex + '">&#x1006;</i>';
                element.tabAdd(tabFilter, {
                    title: title,
                    content: "<iframe class='KLY_iframe' src='" + _this.attr("data-url") + "' data-id='" + tabIdIndex + "'></frame>",
                    id: new Date().getTime()
                })
                //当前窗口内容
                var curmenu = {
                    "icon": _this.find("i").attr("data-icon") != undefined ? _this.find("i").attr("data-icon") : _this.find("i.layui-icon").attr("data-icon"),
