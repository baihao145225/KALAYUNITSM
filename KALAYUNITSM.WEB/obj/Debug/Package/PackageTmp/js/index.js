...
layui.config({
    ...
    $(".hideMenu").hover(function () {
        layer.tips('导航开关', ".hideMenu");
    })

    //隐藏左侧导航
    $(".hideMenu").click(function () {
        $(".layui-layout-admin").toggleClass("showMenu");
        //$(".hideMenu i").addClass("fa fa-angle-double-right");
        //渲染顶部窗口
        tab.tabMove();
    })
    //渲染左侧菜单
    ...
    $("body").on("click", "#unlock", function () {
        ...
        if ($(this).siblings(".admin-header-lock-input").val() == '') {
            ...
            if ($.md5($(this).siblings(".admin-header-lock-input").val()) == password) {
    ...
    $(document).on('keydown', function () {
        var e = document.all ? window.event : arguments[0] ? arguments[0]
                    : event;
    ...
    function showNotice() {
        layer.open({
            ...
            btn: ['我知道了'],
            moveType: 1,
            content: '<div style="padding:10px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p>系统暂不支持手机端打开管理界面，请使用PC端浏览器。</p></div>',
        ...
        for (var i = 0; i < menu.length; i++) {
            ...
            if (menu[i].icon) {
                if (menu[i].icon.split("-")[0] == 'icon') {
                    ...
                    openTitle += '<i class="' + menu[i].icon + '" aria-hidden="true"></i>';
                }
            }
            ...
            element.tabAdd("bodyTab", {
                title: openTitle,
                content: "<iframe class='KLY_iframe' src='" + menu[i].href + "' data-id='" + menu[i].layId + "'></frame>",
                id: menu[i].layId
            })
            //定位到刷新前的窗口
    ...
    //刷新全部
    $(".refreshall").on("click", function () {  //此处添加禁止连续点击刷新一是为了降低服务器压力，另外一个就是为了防止超快点击造成chrome本身的一些js文件的报错(不过貌似这个问题还是存在，不过概率小了很多)
        if ($(this).hasClass("refreshThis")) {
            $(this).removeClass("refreshThis");
            for (var i = 0; i < $(".layui-tab-item").find("iframe").length; i++) {
                $(".layui-tab-item").find("iframe")[i].contentWindow.location.reload(true);
            }
            setTimeout(function () {
                $(".refreshall").addClass("refreshThis");
            }, 10000)
        } else {
            layer.msg("您点击的速度超过了服务器的响应速度，还是等十秒再刷新吧！");
        }
    })
